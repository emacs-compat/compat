...
- Add new Emacs 29 APIs. Most of these are still untested and may change. If you
...
- Add the macros ~compat-call~ and ~compat-function~ to call compatibility
  ...
  using ~compat-function~ and called with ~compat-call~. For example ~(assoc KEY
  ALIST &optional TESTFN)~ can be called with a ~TESTFN~ since Emacs 29. In Emacs
  28 and older the calling convention was ~(assoc KEY ALIST)~. In order to use the
